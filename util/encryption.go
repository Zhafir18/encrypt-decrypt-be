...
func EncryptCBC(plaintext string, encryptKey string) (string, error) {
	keyStr := encryptKey
...
func DecryptCBC(plaintext string, encryptKey string) (string, error) {
	keyStr := encryptKey
	if len(plaintext) < 24 {
	...
	iv, err := base64.URLEncoding.DecodeString(ivBase64)
	if err != nil {
		return "", err
	}
...
func getKey(encryptKey string) ([]byte, error) {
	key := []byte(encryptKey)
	if len(key) != 32 {
...
func getAEAD(encryptKey string) (cipher.AEAD, error) {
...
func EncryptGCM(plaintext string, encryptKey string) (string, error) {
	aead, err := getAEAD(encryptKey)
	if err != nil {
		return "", err
	}
...
func EncryptGCMDeterministic(plaintext string, encryptKey string) (string, error) {
	aead, err := getAEAD(encryptKey)
	if err != nil {
		return "", err
	}
...
func DecryptGCM(encoded string, encryptKey string) (string, error) {
